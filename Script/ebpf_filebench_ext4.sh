...
if [ ! -d ${dist_dir}/${fs}/ebpf ];then
    mkdir ${dist_dir}/${fs}/ebpf
fi
...
do
	...
	do
		...
		sleep 20
		sudo python ${ebpf_file} ${dist_dir}/${fs}/ebpf/ebpf_${fs}_b${block_size}_i${inode_size}_${journal_mode}_t${type}_fileserver_${percent}_${delay}us_${count}.txt 2>/dev/null &
		sleep 10
		sudo killall python
		wait
		...
		sleep 20
		sudo python ${ebpf_file} ${dist_dir}/${fs}/ebpf/ebpf_${fs}_b${block_size}_i${inode_size}_${journal_mode}_t${type}_webserver_${percent}_${delay}us_${count}.txt 2>/dev/null &
		sleep 10
		sudo killall python
		wait
		...
		sleep 20
		sudo python ${ebpf_file} ${dist_dir}/${fs}/ebpf/ebpf_${fs}_b${block_size}_i${inode_size}_${journal_mode}_t${type}_varmail_${percent}_${delay}us_${count}.txt 2>/dev/null &
		sleep 10
		sudo killall python
		wait
